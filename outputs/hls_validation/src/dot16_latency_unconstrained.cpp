...
extern "C" HLS_COMPONENT hls_word_t dot16_latency_unconstrained(hls_word_t in_a0, hls_word_t in_a1, hls_word_t in_a10, hls_word_t in_a11, hls_word_t in_a12, hls_word_t in_a13, hls_word_t in_a14, hls_word_t in_a15, hls_word_t in_a2, hls_word_t in_a3, hls_word_t in_a4, hls_word_t in_a5, hls_word_t in_a6, hls_word_t in_a7, hls_word_t in_a8, hls_word_t in_a9, hls_word_t in_b0, hls_word_t in_b1, hls_word_t in_b10, hls_word_t in_b11, hls_word_t in_b12, hls_word_t in_b13, hls_word_t in_b14, hls_word_t in_b15, hls_word_t in_b2, hls_word_t in_b3, hls_word_t in_b4, hls_word_t in_b5, hls_word_t in_b6, hls_word_t in_b7, hls_word_t in_b8, hls_word_t in_b9) {
    ...
    // op: mul_lut, analytical area=4 latency=6 power=6 dsp=0 lut=8
    hls_word_t n41 = soft_mul(n39, n40);
    ...
    // op: mul_lut, analytical area=4 latency=6 power=6 dsp=0 lut=8
    hls_word_t n44 = soft_mul(n42, n43);
    ...
    // op: mul_lut, analytical area=4 latency=6 power=6 dsp=0 lut=8
    hls_word_t n47 = soft_mul(n45, n46);

    // op: add_dsp, analytical area=2 latency=1 power=2 dsp=1 lut=0
    hls_word_t n48 = dsp_add(n44, n47);

    // op: add_dsp, analytical area=2 latency=1 power=2 dsp=1 lut=0
    hls_word_t n49 = dsp_add(n41, n48);

    // op: add_dsp, analytical area=2 latency=1 power=2 dsp=1 lut=0
    hls_word_t n50 = dsp_add(n38, n49);

    // op: add_dsp, analytical area=2 latency=1 power=2 dsp=1 lut=0
    hls_word_t n51 = dsp_add(n35, n50);

    // op: add_dsp, analytical area=2 latency=1 power=2 dsp=1 lut=0
    hls_word_t n52 = dsp_add(n32, n51);

    // op: add_dsp, analytical area=2 latency=1 power=2 dsp=1 lut=0
    hls_word_t n53 = dsp_add(n29, n52);

    // op: add_dsp, analytical area=2 latency=1 power=2 dsp=1 lut=0
    hls_word_t n54 = dsp_add(n26, n53);

    // op: add_dsp, analytical area=2 latency=1 power=2 dsp=1 lut=0
    hls_word_t n55 = dsp_add(n23, n54);

    // op: add_dsp, analytical area=2 latency=1 power=2 dsp=1 lut=0
    hls_word_t n56 = dsp_add(n20, n55);

    // op: add_dsp, analytical area=2 latency=1 power=2 dsp=1 lut=0
    hls_word_t n57 = dsp_add(n17, n56);

    // op: add_dsp, analytical area=2 latency=1 power=2 dsp=1 lut=0
    hls_word_t n58 = dsp_add(n14, n57);

    // op: add_dsp, analytical area=2 latency=1 power=2 dsp=1 lut=0
    hls_word_t n59 = dsp_add(n11, n58);

    // op: add_dsp, analytical area=2 latency=1 power=2 dsp=1 lut=0
    hls_word_t n60 = dsp_add(n8, n59);

    // op: add_dsp, analytical area=2 latency=1 power=2 dsp=1 lut=0
    hls_word_t n61 = dsp_add(n5, n60);

    // op: add_dsp, analytical area=2 latency=1 power=2 dsp=1 lut=0
    hls_word_t n62 = dsp_add(n2, n61);

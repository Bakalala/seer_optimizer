...
extern "C" HLS_COMPONENT hls_word_t conv3x3_latency_unconstrained(hls_word_t in_p00, hls_word_t in_p01, hls_word_t in_p02, hls_word_t in_p10, hls_word_t in_p11, hls_word_t in_p12, hls_word_t in_p20, hls_word_t in_p21, hls_word_t in_p22) {
    ...
    hls_word_t n5 = dsp_add(n3, n4);

    // op: input, analytical area=0 latency=0 power=0 dsp=0 lut=0
    hls_word_t n6 = in_p00;

    // op: add_dsp, analytical area=2 latency=1 power=2 dsp=1 lut=0
    hls_word_t n7 = dsp_add(n5, n6);

    // op: input, analytical area=0 latency=0 power=0 dsp=0 lut=0
    hls_word_t n8 = in_p02;

    // op: add_dsp, analytical area=2 latency=1 power=2 dsp=1 lut=0
    hls_word_t n9 = dsp_add(n7, n8);
    ...
    // op: add_dsp, analytical area=2 latency=1 power=2 dsp=1 lut=0
    hls_word_t n18 = dsp_add(n14, n17);

    // op: input, analytical area=0 latency=0 power=0 dsp=0 lut=0
    hls_word_t n19 = in_p20;

    // op: add_dsp, analytical area=2 latency=1 power=2 dsp=1 lut=0
    hls_word_t n20 = dsp_add(n18, n19);

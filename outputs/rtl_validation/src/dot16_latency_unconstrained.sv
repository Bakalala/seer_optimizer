...
// analytical metrics: area=94 latency=21 power=126 dsp=15 lut=128
// rtl intended multiplier/MAC DSPs: 0
// expression: ((a15 *lut b15) +dsp ((a14 *lut b14) +dsp ((a13 *lut b13) +dsp ((a12 *lut b12) +dsp ((a11 *lut b11) +dsp ((a10 *lut b10) +dsp ((a9 *lut b9) +dsp ((a8 *lut b8) +dsp ((a7 *lut b7) +dsp ((a6 *lut b6) +dsp ((a5 *lut b5) +dsp ((a4 *lut b4) +dsp ((a3 *lut b3) +dsp ((a2 *lut b2) +dsp ((a0 *lut b0) +dsp (a1 *lut b1))))))))))))))))
module dot16_latency_unconstrained(
    in_a0,
    in_a1,
    in_a10,
    in_a11,
    ...
    // op=mul_lut analytical area=4 latency=6 power=6 dsp=0 lut=8
    logic signed [31:0] n35;
    assign n35 = soft_mul32(n33, n34);

    // op=input analytical area=0 latency=0 power=0 dsp=0 lut=0
    logic signed [31:0] n36;
    assign n36 = in_a3;

    // op=input analytical area=0 latency=0 power=0 dsp=0 lut=0
    logic signed [31:0] n37;
    assign n37 = in_b3;

    // op=mul_lut analytical area=4 latency=6 power=6 dsp=0 lut=8
    logic signed [31:0] n38;
    assign n38 = soft_mul32(n36, n37);

    // op=input analytical area=0 latency=0 power=0 dsp=0 lut=0
    logic signed [31:0] n39;
    assign n39 = in_a2;

    // op=input analytical area=0 latency=0 power=0 dsp=0 lut=0
    logic signed [31:0] n40;
    assign n40 = in_b2;

    // op=mul_lut analytical area=4 latency=6 power=6 dsp=0 lut=8
    logic signed [31:0] n41;
    assign n41 = soft_mul32(n39, n40);

    // op=input analytical area=0 latency=0 power=0 dsp=0 lut=0
    logic signed [31:0] n42;
    assign n42 = in_a0;

    // op=input analytical area=0 latency=0 power=0 dsp=0 lut=0
    logic signed [31:0] n43;
    assign n43 = in_b0;

    // op=mul_lut analytical area=4 latency=6 power=6 dsp=0 lut=8
    logic signed [31:0] n44;
    assign n44 = soft_mul32(n42, n43);

    // op=input analytical area=0 latency=0 power=0 dsp=0 lut=0
    logic signed [31:0] n45;
    assign n45 = in_a1;

    // op=input analytical area=0 latency=0 power=0 dsp=0 lut=0
    logic signed [31:0] n46;
    assign n46 = in_b1;

    // op=mul_lut analytical area=4 latency=6 power=6 dsp=0 lut=8
    logic signed [31:0] n47;
    assign n47 = soft_mul32(n45, n46);

    // op=add_dsp analytical area=2 latency=1 power=2 dsp=1 lut=0
    logic signed [31:0] n48;
    assign n48 = n44 + n47;

    // op=add_dsp analytical area=2 latency=1 power=2 dsp=1 lut=0
    logic signed [31:0] n49;
    assign n49 = n41 + n48;

    // op=add_dsp analytical area=2 latency=1 power=2 dsp=1 lut=0
    logic signed [31:0] n50;
    assign n50 = n38 + n49;

    // op=add_dsp analytical area=2 latency=1 power=2 dsp=1 lut=0
    logic signed [31:0] n51;
    assign n51 = n35 + n50;
    ...
    // op=add_dsp analytical area=2 latency=1 power=2 dsp=1 lut=0
    logic signed [31:0] n53;
    assign n53 = n29 + n52;

    // op=add_dsp analytical area=2 latency=1 power=2 dsp=1 lut=0
    logic signed [31:0] n54;
    assign n54 = n26 + n53;

    // op=add_dsp analytical area=2 latency=1 power=2 dsp=1 lut=0
    logic signed [31:0] n55;
    assign n55 = n23 + n54;

    // op=add_dsp analytical area=2 latency=1 power=2 dsp=1 lut=0
    logic signed [31:0] n56;
    assign n56 = n20 + n55;

    // op=add_dsp analytical area=2 latency=1 power=2 dsp=1 lut=0
    logic signed [31:0] n57;
    assign n57 = n17 + n56;

    // op=add_dsp analytical area=2 latency=1 power=2 dsp=1 lut=0
    logic signed [31:0] n58;
    assign n58 = n14 + n57;

    // op=add_dsp analytical area=2 latency=1 power=2 dsp=1 lut=0
    logic signed [31:0] n59;
    assign n59 = n11 + n58;

    // op=add_dsp analytical area=2 latency=1 power=2 dsp=1 lut=0
    logic signed [31:0] n60;
    assign n60 = n8 + n59;

    // op=add_dsp analytical area=2 latency=1 power=2 dsp=1 lut=0
    logic signed [31:0] n61;
    assign n61 = n5 + n60;

    // op=add_dsp analytical area=2 latency=1 power=2 dsp=1 lut=0
    logic signed [31:0] n62;
    assign n62 = n2 + n61;

  ...
  for mode in $MODE_SET; do
    ...
    (
      ...
      if [[ "${#dsp_args[@]}" -gt 0 ]]; then
        "$IXX" -march="$HLS_TARGET" "${simulator_args[@]}" "${dsp_args[@]}" "$src" -o "$name" \
          > build.log 2>&1
      else
        "$IXX" -march="$HLS_TARGET" "${simulator_args[@]}" "$src" -o "$name" \
          > build.log 2>&1
      fi
    )
  done
...
def render_parse_hls_reports() -> str:
    template = DEFAULT_OUTPUT_DIR / "scripts" / "parse_hls_reports.py"
    if not template.exists():
        raise FileNotFoundError(f"missing HLS report parser template: {template}")
